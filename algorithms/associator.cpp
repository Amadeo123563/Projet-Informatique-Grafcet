...
{
    // Tables pour associer les identifiants � des indices dans les vecteurs
    std::unordered_map<int, int> stepIndices;
    std::unordered_map<int, int> actionIndices;
    std::unordered_map<int, int> transitionIndices;

    // 1. Parcourir les n�uds pour les classer et remplir les tables
    for (size_t i = 0; i < nodes.size(); ++i) {
        ...
        if (node.getType() == Node::Step || node.getType() == Node::InitialStep) {
            ...
            stepIndices[node.getId()] = steps.size(); // Associer l'identifiant � l'index
            steps.push_back(step);
        }
        else if (node.getType() == Node::Action) {
            Action action;
            action.setContent(node.getText());
            actionIndices[node.getId()] = actions.size(); // Associer l'identifiant � l'index
            actions.push_back(action);
        }
        else if (node.getType() == Node::Transition) {
            Transition transition;
            transition.setContent(node.getText());
            transitionIndices[node.getId()] = transitions.size(); // Associer l'identifiant � l'index
            transitions.push_back(transition);
        }
    }

    // 2. Parcourir les ar�tes pour �tablir les relations
    for (size_t i = 0; i < edges.size(); ++i) {
        const Edge& edge = edges[i];
        int sourceId = edge.getSource();
        int destinationId = edge.getDestination();

        // V�rifier si la source est une �tape
        if (stepIndices.find(sourceId) != stepIndices.end()) {
            ...
            // Si la destination est une action, l'ajouter comme action associ�e
            if (actionIndices.find(destinationId) != actionIndices.end()) {
                sourceStep.addAction(actionIndices[destinationId]);
            }
        } // V�rifier si la source est une transition
            ...
            // Si la destination est une �tape, l'ajouter comme �tape suivante
